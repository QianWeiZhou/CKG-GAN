...
from brats_4type_data import ImageFolder
import torch
...
def get_all_data_loaders(conf):
    ...
    if 'data_root' in conf:             
        ...
        train_loader_c = get_data_loader_folder(os.path.join(conf['data_root'], 'train_t1ce'), batch_size, True,
                                              new_size_a, height, width, num_workers, True)
        test_loader_c = get_data_loader_folder(os.path.join(conf['data_root'], 'test_t1ce'), batch_size, False,
                                             new_size_a, new_size_a, new_size_a, num_workers, True)
        train_loader_d = get_data_loader_folder(os.path.join(conf['data_root'], 'train_flair'), batch_size, True,
                                              new_size_b, height, width, num_workers, True)
        test_loader_d = get_data_loader_folder(os.path.join(conf['data_root'], 'test_flair'), batch_size, False,
                                             new_size_b, new_size_b, new_size_b, num_workers, True)


    return train_loader_a, train_loader_b, train_loader_c, train_loader_d, test_loader_a, test_loader_b, test_loader_c, test_loader_d
...
def write_2images(image_outputs, display_image_num, image_directory, postfix):
    n = len(image_outputs)
    # print(n)
    __write_images(image_outputs[0:n//2], display_image_num, '%s/gen-->%s.jpg' % (image_directory, postfix))
    __write_images(image_outputs[n//2:n], display_image_num, '%s/gen<--%s.jpg' % (image_directory, postfix))
