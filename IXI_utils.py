...
from IXI_data import ImageFolder
import torch
...
def get_all_data_loaders(conf):
    ...
    # print(os.path.join(conf['data_root'], 'train_t1_HGG'))
    if 'data_root' in conf:             
        train_loader_a = get_data_loader_folder(os.path.join(conf['data_root'], 'train_t1'), batch_size, True,
                                              new_size_a, height, width, num_workers, True)
        test_loader_a = get_data_loader_folder(os.path.join(conf['data_root'], 'test_t1'), batch_size, False,
                                             new_size_a, new_size_a, new_size_a, num_workers, True)
        train_loader_b = get_data_loader_folder(os.path.join(conf['data_root'], 'train_t2'), batch_size, True,
                                              new_size_b, height, width, num_workers, True)
        test_loader_b = get_data_loader_folder(os.path.join(conf['data_root'], 'test_t2'), batch_size, False,
...
def get_data_loader_folder(input_folder, batch_size, train, new_size=None,
                           height=240, width=240, num_workers=4, crop=True):
...
def write_2images(image_outputs, display_image_num, image_directory, postfix):
    n = len(image_outputs)
    # print(n)
    __write_images(image_outputs[0:n//2], display_image_num, '%s/gen_t1--t2_%s.jpg' % (image_directory, postfix))
    __write_images(image_outputs[n//2:n], display_image_num, '%s/gen_t2--t1_%s.jpg' % (image_directory, postfix))
...
def write_html(filename, iterations, image_save_iterations, image_directory, all_size=1536):
    ...
    write_one_row_html(html_file, iterations, '%s/gen_t1->t2_train_current.jpg' % (image_directory), all_size)
    write_one_row_html(html_file, iterations, '%s/gen_t2->t1_train_current.jpg' % (image_directory), all_size)
    for j in range(iterations, image_save_iterations-1, -1):
        if j % image_save_iterations == 0:
            write_one_row_html(html_file, j, '%s/gen_t1->t2_test_%08d.jpg' % (image_directory, j), all_size)
            write_one_row_html(html_file, j, '%s/gen_t2->t1_test_%08d.jpg' % (image_directory, j), all_size)
            write_one_row_html(html_file, j, '%s/gen_t1->t2_train_%08d.jpg' % (image_directory, j), all_size)
            write_one_row_html(html_file, j, '%s/gen_t2->t1_train_%08d.jpg' % (image_directory, j), all_size)
...
def load_vgg16_4_1():
    model = models.vgg16(pretrained=True).features[:19]
    model.cuda()    
    return model

def load_vgg16_5_1():
    model = models.vgg16(pretrained=True).features[:26]
    model.cuda()    
    return model


def load_shapeNet():
    PATH = './models/best_seg_model.pth'
...
def kd_model_preprocess(batch):
    batch = (batch + 1) * 255 * 0.5
    batch = batch.cuda()
    pixels = batch[batch > 0]
    mean = pixels.mean()
    std = pixels.std()
    batch = (batch - mean) / std
    out_random = np.random.normal(0, 1, size=batch.shape)
    out_random = torch.from_numpy(out_random).float().cuda()
    batch[batch == 0] = out_random[batch == 0]
    return batch
